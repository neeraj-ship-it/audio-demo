...
async function mixProfessionalAudio(layers, options = {}) {
  ...
  try {
    ...
    if (!layers.music && (!layers.effects || layers.effects.length === 0)) {
      ...
      const cmd = `ffmpeg -i "${files.narration}" \
        -af "afade=t=in:st=0:d=${fadeInDuration},afade=t=out:st=5:d=${fadeOutDuration},volume=${narrationVolume}" \
    ...
    // Case 2: Narration + Background Music (most common)
      ...
      const cmd = `ffmpeg -i "${files.narration}" -i "${files.music}" \
        -filter_complex "\
          [0:a]volume=${narrationVolume}[narration]; \
          [1:a]volume=${musicVolume},afade=t=in:st=0:d=${fadeInDuration},afade=t=out:st=5:d=${fadeOutDuration}[music]; \
    ...
    // Case 3: Full professional mix (narration + music + effects)
      ...
      // For now, do narration + music (effects timing requires more complex logic)
      if (layers.music) {
        fs.writeFileSync(files.music, layers.music);
